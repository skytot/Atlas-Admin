...
function analyzeTestResults() {
  ...
  try {
    ...
    console.log('✅ 增强版测试报告已生成: docs/testing/reports.md')
...
function analyzeTestData(results) {
  ...
  results.testResults.forEach(file => {
    ...
    // 处理测试用例详情
    const fileTestCases = file.assertionResults.map(test => ({
      title: test.title,
      status: test.status,
    ...
    module.testCases.push(...fileTestCases)
    analysis.allTestCases.push(...fileTestCases)
...
${index + 1}. **${testCase.title}**
   - 状态: ${testCase.status === 'passed' ? '✅ 通过' : '❌ 失败'}
   - 耗时: ${testCase.duration.toFixed(2)}ms
   - 分组: ${testCase.ancestorTitles.join(' > ')}
   ${testCase.failureMessages.length > 0 ? `- 错误信息: ${testCase.failureMessages[0].split('\\n')[0]}` : ''}
`).join('')}
`).join('')}
...
#### ✅ 通过的测试用例 (${analysis.summary.passedTests}个)

${analysis.allTestCases.filter(tc => tc.status === 'passed').map((testCase, index) => `
${index + 1}. **${testCase.title}**
   - 耗时: ${testCase.duration.toFixed(2)}ms
   - 分组: ${testCase.ancestorTitles.join(' > ')}
`).join('')}
...
#### ❌ 失败的测试用例 (${analysis.summary.failedTests}个)

${analysis.allTestCases.filter(tc => tc.status === 'failed').map((testCase, index) => `
${index + 1}. **${testCase.title}**
   - 耗时: ${testCase.duration.toFixed(2)}ms
   - 分组: ${testCase.ancestorTitles.join(' > ')}
   - 错误信息: ${testCase.failureMessages[0] ? testCase.failureMessages[0].split('\\n')[0] : '无错误信息'}
`).join('')}
` : ''}
